...
| DB_PASSWORD | 数据库密码 | - |
| JWT_SECRET | JWT 密钥 | - |
| TOKEN_SECRET | Token 密钥 | - |
| NACOS_PASSWORD | Nacos 密码 | - |
| DRUID_USERNAME | Druid 控制台用户名 | - |
| DRUID_PASSWORD | Druid 控制台密码 | - |
| FILE_UPLOAD_PATH | 文件上传路径 | ./uploadPath |
| LOG_PATH | 日志路径 | ./logs |
...
2. **方式二：启动参数**
   ```bash
   java -jar ruoyi-admin.jar --DB_PASSWORD=your_password --JWT_SECRET=your_jwt_secret
   ```
